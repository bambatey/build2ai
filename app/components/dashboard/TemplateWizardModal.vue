...
function buildS2KContent(name: string, c: TemplateConfig): string {
  const lines: string[] = []
  lines.push(`; SAP2000 v22 - Generated by build2ai template wizard`)
  lines.push(`; Project: ${name}`)
  lines.push(``)
  lines.push(`TABLE:  "PROGRAM CONTROL"`)
  lines.push(`   ProgramName=SAP2000   Version=22.0.0   ProgLevel=Ultimate`)
  lines.push(``)
  lines.push(`TABLE:  "GRID LINES"`)
  for (let i = 0; i < c.gridX; i++) {
    lines.push(`   CoordSys=GLOBAL  AxisDir=X  GridID=X${i + 1}  Ordinate=${(i * c.gridSpacing).toFixed(2)}`)
  }
  for (let i = 0; i < c.gridY; i++) {
    lines.push(`   CoordSys=GLOBAL  AxisDir=Y  GridID=Y${i + 1}  Ordinate=${(i * c.gridSpacing).toFixed(2)}`)
  }
  for (let i = 0; i <= c.numFloors; i++) {
    lines.push(`   CoordSys=GLOBAL  AxisDir=Z  GridID=Z${i}  Ordinate=${(i * c.floorHeight).toFixed(2)}`)
  }
  lines.push(``)
  lines.push(`TABLE:  "MATERIAL PROPERTIES"`)
  for (const m of c.materials) {
    const isConcrete = m.startsWith('C')
    lines.push(`   Material=${m}  Type=${isConcrete ? 'Concrete' : 'Steel'}`)
  }
  lines.push(``)
